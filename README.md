...
Примечание: на Android 12+ для доступа к списку устройств и подключений могут требоваться разрешения Bluetooth (BLUETOOTH_CONNECT, BLUETOOTH_SCAN).

## Рекомендации по скорости
- Для максимальной скорости держите соединение открытым: `УстановитьТаймаутОтключения(0)` и `ПодключитьПринтер` один раз.
- Используйте `ИспользоватьНебезопасныйСокет(Истина)` для более быстрого подключения (может быть менее совместимо).
- Старайтесь печатать короткие строки и избегать больших изображений/QR на слабых принтерах.
